...
// ============================================================
// READ-ONLY инспекция слоя + рендер кадра (добавлено для отладки
// переходов панели "Включайся!"). Существующие команды не трогают.
// ============================================================
function bridgeResolveComp(args) {
    if (args && args.compId) {
        for (var i = 1; i <= app.project.numItems; i++) {
            var it = app.project.item(i);
            if (it instanceof CompItem && it.id === args.compId) return it;
        }
        throw new Error("No composition with id " + args.compId);
    }
    var name = (args && args.compName) ? args.compName : "";
    if (name) {
        var best = null;
        for (var j = 1; j <= app.project.numItems; j++) {
            var c = app.project.item(j);
            if (c instanceof CompItem && c.name === name) {
                if (!best || c.numLayers > best.numLayers) best = c;
            }
        }
        if (best) return best;
    ...
    throw new Error("No comp: pass compId or compName");
}

function bridgeMaskModeName(mm) {
...
function inspectLayer(args) {
    try {
        var comp = bridgeResolveComp(args);
        var layerName = (args && args.layerName) ? args.layerName : "";
        var layer = null;
        for (var i = 1; i <= comp.numLayers; i++) {
            if (comp.layer(i).name === layerName) { layer = comp.layer(i); break; }
        }
        if (!layer) throw new Error("Layer '" + layerName + "' not found in comp '" + comp.name + "' (id " + comp.id + ")");
        ...
        try {
            ...
            srcRect = { top: r.top, left: r.left, width: r.width, height: r.height };
        } catch (eR) { srcRect = { error: eR.toString() }; }

        var masks = [];
        var parade = null;
        try { parade = layer.property("ADBE Mask Parade"); } catch (eP) {}
        if (parade) {
            for (var m = 1; m <= parade.numProperties; m++) {
                var mk = parade.property(m);
                var verts = null;
                try {
                    var sh = mk.property("ADBE Mask Shape").value;
                    verts = [];
                    for (var v = 0; v < sh.vertices.length; v++) verts.push([sh.vertices[v][0], sh.vertices[v][1]]);
                } catch (eV) {}
                var expansion = null;
                try { expansion = mk.property("ADBE Mask Offset").value; } catch (eO) {}
                masks.push({
                    name: mk.name,
                    mode: bridgeMaskModeName(mk.maskMode),
        ...
        return JSON.stringify({
            status: "success",
            comp: { id: comp.id, name: comp.name, width: comp.width, height: comp.height, numLayers: comp.numLayers, duration: comp.duration },
            layer: {
                name: layer.name,
                index: layer.index,
                enabled: layer.enabled,
                position: posProp.value,
                positionNumKeys: posProp.numKeys,
                ...
                source: layer.source ? { name: layer.source.name, width: layer.source.width, height: layer.source.height } : null,
                sourceRect: srcRect,
                masks: masks
            }
        });
...
// Восстановление руч. прямоугольной маски (после того как баг в панели её
// снёс). args: {compId|compName, layerName, name?, left, top, width, height}
function createRectMask(args) {
    try {
        var comp = bridgeResolveComp(args);
        var layerName = (args && args.layerName) ? args.layerName : "";
        var layer = null;
        ...
        if (!layer) throw new Error("Layer '" + layerName + "' not found in comp '" + comp.name + "'");
        var nm = (args && args.name) ? args.name : "Mask 1";
        var L = args.left, T = args.top, W = args.width, H = args.height;

        var masks = layer.property("ADBE Mask Parade");
        for (var m = masks.numProperties; m >= 1; m--) {
            if (masks.property(m).name === nm) masks.property(m).remove();
        }
        ...
        return JSON.stringify({ status: "success", layer: layer.name, mask: nm,
            rect: [L, T, W, H], numMasks: masks.numProperties });
    } catch (error) {
        return JSON.stringify({ status: "error", message: error.toString() });
    }
}

// Список ВСЕХ слоёв композиции (comp по имени/id — не обязана быть активной).
function listLayers(args) {
    try {
        var comp = bridgeResolveComp(args);
        var out = { status: "success", comp: { id: comp.id, name: comp.name, width: comp.width, height: comp.height, duration: comp.duration, numLayers: comp.numLayers }, layers: [] };
        for (var i = 1; i <= comp.numLayers; i++) {
            var l = comp.layer(i);
            out.layers.push({
                index: l.index, name: l.name, enabled: l.enabled, locked: l.locked,
...
function saveFrame(args) {
    try {
        ...
        var dir = new Folder(Folder.myDocuments.fsName + "/ae-mcp-bridge/frames");
        if (!dir.exists) dir.create();
        var base = (args && args.outName) ? args.outName
            : ("frame_" + String(comp.name).replace(/[^\w\-]+/g, "_") + "_t" + String(time).replace(".", "p"));
        var f = new File(dir.fsName + "/" + base + ".png");
        comp.saveFrameToPng(time, f);
        return JSON.stringify({ status: "success", path: f.fsName, comp: comp.name, time: time });
    } catch (error) {
        return JSON.stringify({ status: "error", message: error.toString() });
    }
}
    ...
    try {
        ...
        switch (command) {
            ...
            case "bridgeTestEffects":
                ...
                break;
            case "inspectLayer":
                logToPanel("Calling inspectLayer function...");
                result = inspectLayer(args);
                logToPanel("Returned from inspectLayer.");
                break;
            case "listLayers":
                logToPanel("Calling listLayers function...");
                result = listLayers(args);
                logToPanel("Returned from listLayers.");
                break;
            case "saveFrame":
                ...
                break;
            case "createRectMask":
                logToPanel("Calling createRectMask function...");
                result = createRectMask(args);
                logToPanel("Returned from createRectMask.");
                break;
            default:
